...
class Methods:
  ...
  def edittext(self, text, chat_id=None, message_id=None, **kwargs):
    kwargs['text'] = text
    kwargs['chat_id'] = self.get_chat_id(chat_id) if not chat_id else chat_id
    kwargs['message_id'] = self.get_message_id(message_id) if not message_id else message_id
    return self.method('editMessageText', **kwargs)
    
  def editcaption(self, caption=None, chat_id=None, message_id=None, **kwargs):
